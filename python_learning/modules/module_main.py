...
# 导入同一个包中的模块可以使用相对路径，对于 `import module1 as md1`，如果所有代码完全在同一个包中执行是可行的，
# 但如果在不同一个包中，这将报错，无法找到对应模块。
# 这并不意味着必须使用绝对路径，而是要正确使用相对路径（注意：此模块被多次导入，但其内部可执行语句只执行了一次）：
...
# 通常不建议使用 * 导入，因为你不知道导入了什么，如果是在交互模式下，那当我没说。
